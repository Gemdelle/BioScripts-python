...
class SmallTree1:
    def __init__(self):
        self.frames = {}
        self.frame_index = 0
        self.rect = pygame.Rect(0, 0, Constants.SMALL_TREE_1_SIZE, Constants.SMALL_TREE_1_SIZE)
    ...
    def load_frame(self, index):
        if index not in self.frames:
            frame_path = os.path.join("./assets/gifs/frames/small-tree-1", f'small-tree-1_{index}.png')
            if os.path.exists(frame_path):
                surf = pygame.image.load(frame_path).convert_alpha()
                surf = pygame.transform.scale(surf, (Constants.SMALL_TREE_1_SIZE, Constants.SMALL_TREE_1_SIZE))
    ...
    def load_gif_frames(self):
        for i in range(29):  # Assuming the GIF has 24 frames
            frame_path = os.path.join("./assets/gifs/frames/harvest-small-tree", f'harvest-small-tree_{i}.png')
            if os.path.exists(frame_path):
                surf = pygame.image.load(frame_path).convert_alpha()
    ...
    def draw(self, screen, x, y):
        self.rect.x = x + Constants.SMALL_TREE_1_SIZE
        self.rect.y = y + Constants.SMALL_TREE_1_SIZE
    ...
    def harvest(self, wrong_command, on_successful_harvest):
        ...
        on_successful_harvest("small-tree")
