...
class MushroomPlant:
    ...
    def load_frame(self, index):
        if index not in self.frames:
            frame_path = os.path.join("./assets/gifs/frames/mushroom-default", f'mushroom-default_{index}.png')
    ...
    def load_gif_frames(self):
        for i in range(29):  # Assuming the GIF has 24 frames
            frame_path = os.path.join("./assets/gifs/frames/harvest-mushroom", f'harvest-mushroom_{i}.png')
            if os.path.exists(frame_path):
                surf = pygame.image.load(frame_path).convert_alpha()
                surf = pygame.transform.scale(surf, (Constants.MUSHROOM_SIZE, Constants.MUSHROOM_SIZE))
    ...
    def update_animation(self):
        ...
        if self.visible:
            self.frame_index = (current_time // Constants.FRAME_DURATION_IN_MILLIS) % 24  # Assuming 200 frames
    ...
    def harvest(self, wrong_command, on_successful_harvest):
        ...
        on_successful_harvest("mushroom")
