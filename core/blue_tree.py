...
class BlueTree:
    def __init__(self):
        self.frames = {}
        self.frame_index = 1
        self.rect = pygame.Rect(0, 0, Constants.BLUE_TREE_SIZE, Constants.BLUE_TREE_SIZE)
        self.is_colliding = False
        self.tooltip_text = "blue tree"
    ...
    def load_frame(self, index):
        if index not in self.frames:
            frame_path = os.path.join("./assets/gifs/frames/tree-blue", f'tree-blue_{index}.png')
            if os.path.exists(frame_path):
                surf = pygame.image.load(frame_path).convert_alpha()
                surf = pygame.transform.scale(surf, (Constants.BLUE_TREE_SIZE, Constants.BLUE_TREE_SIZE))
    ...
    def draw(self, screen, x, y):
        self.rect.x = x + Constants.BLUE_TREE_SIZE
        self.rect.y = y + Constants.BLUE_TREE_SIZE
