...
class AnalyzeDiseaseBar:
    ...
    def load_frame(self, index):
        if index not in self.frames:
            frame_path = os.path.join("./assets/gifs/frames/plants-disease-bar", f'plants-disease-bar_{index}.png')
            if os.path.exists(frame_path):
                surf = pygame.image.load(frame_path).convert_alpha()
                surf = pygame.transform.scale(surf, (600, 200)) # scale
    ...
    def update_animation(self):
        ...
        self.frame_index = (current_time // Constants.FRAME_DURATION_IN_MILLIS) % 138  # Assuming 200 frames
