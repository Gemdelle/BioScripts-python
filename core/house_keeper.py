...
class Housekeeper:
    ...
    def load_frames(self):
        if not self.frames:
            frame_index = 1
            while True:
                frame_path = os.path.join("./assets/gifs/frames/housekeeper_happy", f'housekeeper_happy_{frame_index}.png')
                if not os.path.exists(frame_path):
                    break
                surf = pygame.image.load(frame_path).convert_alpha()
                surf = pygame.transform.scale(surf, (Constants.HOUSE_KEEPER_SIZE, Constants.HOUSE_KEEPER_SIZE))
                self.frames.append(surf)
                frame_index += 1
                self.rect.width = Constants.HOUSE_KEEPER_SIZE
                self.rect.height = Constants.HOUSE_KEEPER_SIZE
    ...
    def draw(self, screen, x, y):
        self.rect.x = x + Constants.HOUSE_KEEPER_SIZE
        self.rect.y = y + Constants.HOUSE_KEEPER_SIZE
