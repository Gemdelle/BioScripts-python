...
class Enemy:
    def __init__(self):
        self.frames = []
        self.frame_index = 0
        self.rect = pygame.Rect(0, 0, Constants.ENEMY_SIZE, Constants.ENEMY_SIZE)
        self.is_colliding = False
        self.tooltip_text = "enemy"
    ...
    def load_frames(self):
        frame_index = 1
        while True:
            frame_path = os.path.join("./assets/gifs/frames/enemy", f'enemy_{frame_index}.png')
            if not os.path.exists(frame_path):
                break
            surf = pygame.image.load(frame_path).convert_alpha()
            surf = pygame.transform.scale(surf, (Constants.ENEMY_SIZE, Constants.ENEMY_SIZE))
            self.frames.append(surf)
            frame_index += 1
            self.rect.width = Constants.ENEMY_SIZE
            self.rect.height = Constants.ENEMY_SIZE
    ...
    def update_animation(self):
        ...
        if self.is_colliding:
            ...
            self.current_text_length = 0  # Reset text length when not colliding

    def draw(self, screen, x, y):
        self.rect.x = x + Constants.ENEMY_SIZE
        self.rect.y = y + Constants.ENEMY_SIZE
    ...
    def talk(self, start_tkinter_app):
