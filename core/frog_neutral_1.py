...
class FrogNautral1:
    def __init__(self):
        self.frames = []
        self.frame_index = 0
        self.rect = pygame.Rect(0, 0, Constants.FROG_NEUTRAL_1_SIZE, Constants.FROG_NEUTRAL_1_SIZE)
    ...
    def load_frames(self):
        frame_index = 1
        while True:
            frame_path = os.path.join("./assets/gifs/frames/frog_neutral_1", f'frog_neutral_1_{frame_index}.png')
            if not os.path.exists(frame_path):
                break
            surf = pygame.image.load(frame_path).convert_alpha()
            surf = pygame.transform.scale(surf, (Constants.FROG_NEUTRAL_1_SIZE, Constants.FROG_NEUTRAL_1_SIZE))
            self.frames.append(surf)
            frame_index += 1
            self.rect.width = Constants.FROG_NEUTRAL_1_SIZE
            self.rect.height = Constants.FROG_NEUTRAL_1_SIZE
    ...
    def draw(self, screen, x, y):
        self.rect.x = x + Constants.FROG_NEUTRAL_1_SIZE
        self.rect.y = y + Constants.FROG_NEUTRAL_1_SIZE
