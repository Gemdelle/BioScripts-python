...
class FrogHappy:
    def __init__(self):
        self.frames = {}
        self.frame_index = 1
        self.rect = pygame.Rect(0, 0, Constants.FROG_HAPPY_SIZE, Constants.FROG_HAPPY_SIZE)
    ...
    def load_frame(self, index):
        if index not in self.frames:
            frame_path = os.path.join("./assets/gifs/frames/frog_happy", f'frog_happy_{index}.png')
            if os.path.exists(frame_path):
                surf = pygame.image.load(frame_path).convert_alpha()
                surf = pygame.transform.scale(surf, (Constants.FROG_HAPPY_SIZE, Constants.FROG_HAPPY_SIZE))
    ...
    def draw(self, screen, x, y):
        self.rect.x = x + Constants.FROG_HAPPY_SIZE
        self.rect.y = y + Constants.FROG_HAPPY_SIZE
